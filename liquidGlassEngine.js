        ...
        this._signals = [];
            ...
            this.set_shader_source(`
                ...
                void main(void) {
                    ...
                    float shapeAlpha = 1.0 - outsideTransition;
                    if (shapeAlpha <= 0.0) {
                    ...
                    vec4 baseColor;
                    if (is_dark > 0.5) {
                        baseColor = vec4(0.09, 0.12, 0.18, 0.22);
                    } else {
                        baseColor = vec4(0.96, 0.98, 1.0, 0.12);
                    }
                    ...
                    baseColor.rgb *= (1.0 - aoMask * aoIntensity);
                    ...
                    vec3 litColor = baseColor.rgb + addedLight - (baseColor.rgb * addedLight);

                    float lightLuma = max(addedLight.r, max(addedLight.g, addedLight.b));
                    float finalAlpha = clamp(baseColor.a + lightLuma, 0.0, 1.0);
                    ...
                    cogl_color_out = vec4(litColor * finalAlpha, finalAlpha) * shapeAlpha;
                }
            `);
...
export const LiquidGlassSurface = GObject.registerClass({
    ...
    _init({
        ...
        super._init({
            ...
            y_expand: true,
            reactive: false,
            // Add a tiny transparent background so Shell.BlurEffect knows we want to blur
            style: 'background-color: rgba(255, 255, 255, 0.01);'
        });
        this._initialParams = { sigma, brightness, radius, refraction, dispersion, specular, darkTheme };
    }

    initialize() {
        if (!this._initialParams) return;
        const { sigma, brightness, radius, refraction, dispersion, specular, darkTheme } = this._initialParams;
    ...
    update(params = {}) {
        ...
        const { sigma, brightness, radius, refraction, dispersion, specular, darkTheme } = params;

        if (this._blurEffect) {
            if (sigma !== undefined) this._blurEffect.radius = sigma * 2 * this._scale;
            if (brightness !== undefined) this._blurEffect.brightness = brightness;
            this._blurEffect.queue_repaint();
        }

        if (this._glassEffect) {
            if (radius !== undefined) this._glassEffect.radius = radius * this._scale;
            if (refraction !== undefined) this._glassEffect.refraction = refraction;
            if (dispersion !== undefined) this._glassEffect.dispersion = dispersion;
            if (specular !== undefined) this._glassEffect.specular = specular;
            if (darkTheme !== undefined) this._glassEffect.darkTheme = darkTheme;
        }
    }
    ...
    initialize() {
        ...
        // The shader effect is applied to the parent container (this)
        this._glassEffect = new LiquidGlassEffect({
            radius: radius * this._scale,
            scale: this._scale,
            refraction,
        ...
        // The blur effect is applied to a child widget. 
        // This forces Shell.BlurEffect to draw the blurred background into the parent's Offscreen FBO,
        // which is then passed as 'tex' to the glass shader!
        this._blurChild = new St.Widget({
            name: 'maclike-liquid-blur-child',
            x_expand: true,
            y_expand: true,
        ...
        this._blurChild.add_effect(this._blurEffect);
        this.add_child(this._blurChild);
        this.add_effect(this._glassEffect);
    }
